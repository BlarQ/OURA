  ...
  const modalContent = (
    ...
    >
      ...
      >
        ...
        <div className="p-2.5 rounded-[8px] bg-[#f8fafc] border border-[#d9d9d9] space-y-1">
          <div className="flex items-center gap-2 text-[10px] text-[#6e797a] font-bold">
            <span>Date: {activity.activity_date}</span>
            {activity.time_slot && <span>• {activity.time_slot}</span>}
          </div>
          <p className="text-xs font-extrabold text-[#040404] leading-snug line-clamp-2">
            {activity.title}
          </p>
        </div>
        ...
        <div className="space-y-2.5">
          ...
          <div className="space-y-1">
            {suggestions.map((sug) => (
              <button
                ...
                className={`w-full p-2 rounded-[6px] text-xs font-bold text-left border transition cursor-pointer flex items-center justify-between ${
                  (selectedDate || suggestions[0]?.dateStr) === sug.dateStr
                    ? 'bg-[#040404] text-[#ffffff] border-[#040404]'
                    : 'bg-[#ffffff] text-[#040404] border-[#d9d9d9] hover:border-[#040404]'
                }`}
              >
                <span>{sug.label}</span>
          ...
          <div className="space-y-1 pt-0.5">
            <label className="text-[10px] font-bold text-[#6e797a] block">
          ...
          <div className="pt-0.5">
            <label className="flex items-center gap-2 cursor-pointer select-none text-[11px] text-[#040404] font-bold">
              <input
                type="checkbox"
                checked={resetCompleted}
                onChange={(e) => setResetCompleted(e.target.checked)}
                className="h-3.5 w-3.5 rounded accent-[#040404]"
              />
