...
export default function WelcomeBanner({ userName, manualsCount }: WelcomeBannerProps) {
  ...
  return (
    <div className="relative hero-reversed-panel p-5 sm:p-7 space-y-4 sm:space-y-5 animate-modal-in shadow-xl overflow-hidden">
      ...
      <div className="absolute -right-20 -bottom-20 w-64 h-64 bg-[#98e58e]/10 rounded-full blur-3xl pointer-events-none" />

      {/* Dismiss Button */}
      <button
        type="button"
        onClick={() => setIsVisible(false)}
        className="absolute top-3 right-3 sm:top-3.5 sm:right-3.5 p-1.5 text-[#6e797a] hover:text-[#ffffff] rounded-[6px] hover:bg-white/10 transition cursor-pointer active:scale-90"
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 sm:gap-5 relative z-10">
        <div className="space-y-2.5 max-w-xl pr-6 sm:pr-0">
          ...
          <div className="badge-sprout-green text-[10px] sm:text-xs py-0.5 px-2.5 whitespace-nowrap inline-flex items-center gap-1.5 max-w-full">
            <Terminal className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-[#040404] shrink-0" />
          ...
          <h2 className="text-xl sm:text-2xl md:text-3xl font-extrabold text-[#ffffff] tracking-tight leading-snug break-words">
            {greeting}, {userName}
          </h2>

          <p className="text-xs sm:text-sm text-[#cbcece] leading-relaxed">
          ...
          {/* Quick Metrics */}
          <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 pt-0.5 text-xs text-[#ffffff]">
            <span className="badge-sprout-neutral bg-white/10 border-white/20 text-white hover:bg-white/20 transition-colors whitespace-nowrap text-[10px] sm:text-xs py-0.5 px-2 sm:px-2.5">
              <Layers className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-[#98e58e]" />
              <span>{manualsCount} {manualsCount === 1 ? 'Procedure' : 'Procedures'}</span>
            </span>
            <span className="badge-sprout-neutral bg-white/10 border-white/20 text-white hover:bg-white/20 transition-colors whitespace-nowrap text-[10px] sm:text-xs py-0.5 px-2 sm:px-2.5">
              <ShieldCheck className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-[#98e58e]" />
              <span>Row-Level Security</span>
            </span>
            <span className="badge-sprout-neutral bg-white/10 border-white/20 text-white hover:bg-white/20 transition-colors whitespace-nowrap text-[10px] sm:text-xs py-0.5 px-2 sm:px-2.5">
              <Activity className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-[#98e58e]" />
