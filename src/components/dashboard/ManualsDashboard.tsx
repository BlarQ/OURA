...
export default function ManualsDashboard({
  ...
  return (
    <div className="relative min-h-screen bg-[#ffffff] text-[#040404] font-sans flex flex-col justify-between selection:bg-[#98e58e] selection:text-[#040404] overflow-x-hidden">
      ...
      <div className="relative z-10">
        ...
        <header className="sticky top-2 sm:top-4 z-30 max-w-[1200px] mx-auto px-3 sm:px-6 my-2 sm:my-3">
          <div className="nav-sprout h-14 sm:h-16 px-3.5 sm:px-5 flex items-center justify-between backdrop-blur-md bg-white/95">
            ...
            >
              <BrandLogo size="md" />
              <span className="text-lg sm:text-xl font-extrabold tracking-tight text-[#040404]">
                AdeManual
              </span>
            </Link>

            {/* Desktop Navigation Tabs Switcher with Sliding Active Indicator */}
            <div className="hidden sm:flex items-center relative bg-[#f4f4f4] p-1 rounded-[8px] border border-[#d9d9d9]">
              <button
                type="button"
                onClick={() => setActiveTab('planner')}
                className={`relative z-10 px-3.5 sm:px-4 py-1.5 text-xs font-bold rounded-[6px] transition-all duration-200 cursor-pointer flex items-center gap-1.5 ${
                  activeTab === 'planner'
                    ? 'text-white bg-[#040404] shadow-sm'
            ...
            <div className="flex items-center gap-2 sm:gap-3 text-sm shrink-0">
              <div
                className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1 rounded-[24px] bg-[#f4f4f4] border border-[#d9d9d9] text-[#040404] font-bold text-xs hover:border-[#040404] transition-colors whitespace-nowrap"
                title={userName || userEmail || 'Adedamola Ogunlala'}
              >
                <span className="w-2 h-2 rounded-full bg-[#98e58e] shrink-0 animate-pulse" />
              ...
              <button
                ...
                className="p-1 sm:p-0 text-[13px] font-bold text-[#6e797a] hover:text-[#040404] transition cursor-pointer flex items-center gap-1 hover:translate-x-0.5"
                title="Sign out"
              >
        ...
        <div className="sm:hidden max-w-[1200px] mx-auto px-3 my-1.5">
          <div className="flex items-center gap-1 bg-[#f4f4f4] p-1 rounded-[8px] border border-[#d9d9d9]">
            <button
              type="button"
              onClick={() => setActiveTab('planner')}
              className={`flex-1 py-1.5 text-xs font-bold rounded-[6px] transition-all duration-200 cursor-pointer flex items-center justify-center gap-1.5 ${
                activeTab === 'planner'
                  ? 'bg-[#040404] text-white shadow-sm'
                  : 'text-[#6e797a] hover:text-[#040404]'
              }`}
            >
              <CalendarDays className="h-3.5 w-3.5" />
        ...
        <main className="max-w-[1200px] mx-auto px-3 sm:px-6 py-4 sm:py-8 space-y-6 sm:space-y-8">
          {/* Success Notification */}
          {notification && (
            <div className="p-3.5 sm:p-4 rounded-[6px] bg-[#98e58e]/20 border border-[#98e58e] flex items-center justify-between text-xs font-bold text-[#040404] animate-modal-in">
              <div className="flex items-center gap-2">
                <CheckCircle2 className="h-4 w-4 text-[#040404]" />
                <span>{notification}</span>
              </div>
              <button
                type="button"
                onClick={() => setNotification(null)}
                className="p-1 text-[#040404] hover:opacity-75 cursor-pointer"
              >
          ...
          {activeTab === 'planner' && (
            <div key="tab-planner" className="space-y-4 sm:space-y-6 animate-tab-content">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 sm:gap-4 border-b border-[#d9d9d9] pb-3 sm:pb-4">
                <div className="space-y-0.5 sm:space-y-1 min-w-0">
                  <div className="flex items-center gap-2 flex-nowrap">
                    <h2 className="text-base sm:text-2xl md:text-3xl font-extrabold text-[#040404] tracking-tight whitespace-nowrap">
          ...
          {activeTab === 'manuals' && (
            <div key="tab-manuals" className="space-y-4 sm:space-y-6 animate-tab-content">
              {/* Section Header */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 border-b border-[#d9d9d9] pb-4 sm:pb-5">
                <div className="space-y-0.5 sm:space-y-1">
                  <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                    <h2 className="text-xl sm:text-2xl md:text-3xl font-extrabold text-[#040404] tracking-tight whitespace-nowrap">
                  ...
                  <p className="text-xs sm:text-sm text-[#6e797a]">
              ...
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 sm:pl-3.5 flex items-center pointer-events-none text-[#6e797a]">
                ...
                {searchQuery && (
                  <button
                    ...
                    className="absolute inset-y-0 right-0 pr-3 sm:pr-3.5 flex items-center text-[#6e797a] hover:text-[#040404] cursor-pointer"
                  >
                    <X className="h-4 w-4" />
                  </button>
                )}
              ...
              <div className="space-y-3 sm:space-y-4">
                {filteredManuals.length === 0 ? (
                  <div className="card-sprout p-8 sm:p-12 text-center space-y-3 sm:space-y-4 border-dashed">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-[6px] bg-[#f4f4f4] border border-[#d9d9d9] text-[#040404] mx-auto flex items-center justify-center">
                      <FileText className="h-5 w-5 sm:h-6 sm:w-6" />
                    </div>
                    <div className="space-y-1">
                      <h3 className="text-base sm:text-lg font-extrabold text-[#040404]">
                        {searchQuery ? 'No matching procedures found' : 'No procedure manuals yet'}
                      </h3>
                      <p className="text-xs text-[#6e797a] max-w-sm mx-auto">
                        {searchQuery
                  ...
                  filteredManuals.map((item, idx) => (
                    <div
                      key={item.id}
                      className="card-sprout p-4 sm:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 transition-all duration-200 hover:border-[#040404] hover:-translate-y-0.5 hover:shadow-md animate-tab-content"
                      style={{ animationDelay: `${idx * 0.05}s` }}
                    >
                      <div className="space-y-1.5 sm:space-y-2 flex-1 min-w-0 pr-0 sm:pr-4">
                        <Link
                          href={`/manuals/${item.id}`}
                          className="text-base sm:text-xl font-extrabold text-[#040404] hover:text-[#040404] link-sprout block truncate"
                        >
                          {item.title}
                        </Link>
                        <p className="text-xs sm:text-sm text-[#6e797a] line-clamp-2 leading-relaxed">
                          {item.description_preview ||
                            'Sequential operating steps, command lines, and media documentation.'}
                        </p>
                        <div className="flex items-center gap-2 sm:gap-3 pt-0.5 text-[11px] sm:text-xs text-[#6e797a] font-bold">
                      ...
                      <div className="flex items-center gap-1.5 sm:gap-2 self-end sm:self-center shrink-0">
                        <Link
                          ...
                          className="p-1.5 sm:p-2 text-[#6e797a] hover:text-[#040404] hover:bg-[#f4f4f4] rounded-[6px] transition cursor-pointer border border-transparent hover:border-[#d9d9d9]"
                          title="Edit Manual"
                        >
                        ...
                        <button
                          ...
                          className="p-1.5 sm:p-2 text-[#6e797a] hover:text-red-600 hover:bg-red-50 rounded-[6px] transition cursor-pointer border border-transparent hover:border-red-200"
                          title="Delete Manual"
                        >
      ...
      {/* Footer */}
      <footer className="relative z-10 max-w-[1200px] w-full mx-auto px-4 sm:px-6 py-6 sm:py-8 text-xs text-[#6e797a] flex items-center justify-between border-t border-[#d9d9d9] mt-8 sm:mt-12 bg-white/80 backdrop-blur-xs">
        <div className="flex items-center gap-2">
        ...
        <button
          ...
          className="link-sprout text-[11px] text-[#6e797a] hover:text-[#040404]"
        >
          Admin Access
        </button>
      </footer>
