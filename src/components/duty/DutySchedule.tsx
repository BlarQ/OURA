...
export const DutySchedule: React.FC = () => {
  ...
  const getDutyBadge = (dutyType: 'Morning' | 'Night' | 'OFF') => {
    switch (dutyType) {
      case 'Morning':
        return <span className="bg-amber-100 text-amber-800 font-bold px-2.5 py-1 rounded-full text-xs flex items-center gap-1"><Sun className="w-3.5 h-3.5" /> Morning Shift</span>;
      case 'Night':
        return <span className="bg-indigo-900 text-indigo-100 font-bold px-2.5 py-1 rounded-full text-xs flex items-center gap-1"><Moon className="w-3.5 h-3.5" /> Night Shift</span>;
      case 'OFF':
        return <span className="bg-emerald-100 text-emerald-800 font-bold px-2.5 py-1 rounded-full text-xs flex items-center gap-1"><Coffee className="w-3.5 h-3.5" /> OFF Duty</span>;
    }
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-3xl p-6 shadow-xl border border-slate-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-xl font-black text-slate-900 tracking-tight">WORK DUTY ROTATION ENGINE</h2>
              <p className="text-xs text-slate-500">
                {currentRole === 'husband'
                  ? `Select your Work From Home (WFH) days to sync with ${wifeProfile.name}'s schedule`
        ...
        <button
          ...
          className="flex items-center gap-1.5 bg-[#695be8] hover:bg-indigo-700 text-white font-extrabold text-xs px-4 py-2.5 rounded-2xl transition-all shadow-md active:scale-95 whitespace-nowrap"
        >
          <Settings className="w-4 h-4" /> {showSetup ? 'Hide Setup' : currentRole === 'husband' ? 'Set WFH Days' : 'Configure Shift Baseline'}
        </button>
      </div>

      {/* TOP CONFIGURATION CARD (Role-Specific) */}
      {showSetup && (
        <div className="bg-indigo-50 border border-indigo-200 rounded-3xl p-5 shadow-sm space-y-3 animate-fadeInScale">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-extrabold text-indigo-950 flex items-center gap-1.5">
              ...
              {currentRole === 'husband' ? 'CONFIGURE HUSBAND WORK FROM HOME (WFH) DAYS' : 'CONFIGURE SHIFT BASELINE'}
            </h3>
            {saveSuccess && (
          ...
          <form onSubmit={handleSaveSetup} className="space-y-3 text-xs">
            {currentRole === 'husband' ? (
              /* Husband WFH Day Selection Form */
              <div className="space-y-2">
                <label className="font-bold text-slate-700 block">
                  Select the day(s) of the week you work from home or are free:
                </label>
                ...
                <p className="text-[11px] text-slate-500 font-medium pt-1">
                  Selected WFH days will automatically display as <span className="font-bold text-indigo-700">🏠 Husband WFH</span> on {wifeProfile.name}'s schedule!
                </p>
              </div>
            ) : (
              /* Wife's 2-Day Shift Baseline Form */
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {/* Day 1 Entry */}
                <div className="p-3 bg-white rounded-2xl border border-indigo-100 space-y-2">
                  <span className="font-extrabold text-slate-800 text-[11px] block">Day 1 Entry</span>
                ...
                {/* Day 2 Entry */}
                <div className="p-3 bg-white rounded-2xl border border-indigo-100 space-y-2">
                  <span className="font-extrabold text-slate-800 text-[11px] block">Day 2 Entry</span>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="text-[10px] font-bold text-slate-500 block mb-1">Date</label>
                      <input
            ...
            <div className="flex justify-end pt-1">
              ...
              >
                {currentRole === 'husband' ? 'Save & Sync WFH Days' : 'Calculate 30-Day Rotation Schedule'}
              </button>
            </div>
          </form>
        </div>
      ...
      <div className="bg-gradient-to-br from-indigo-900 via-indigo-950 to-slate-900 text-white rounded-3xl p-6 shadow-2xl space-y-4 border border-indigo-800">
        <div className="flex items-center justify-between">
          <span className="text-xs font-bold uppercase tracking-wider text-indigo-300">TODAY'S SHIFT SUMMARY</span>
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-indigo-200 mt-1">
              {getDutySummaryText(dutyDays[0], wifeProfile.name)}
            </p>
          </div>
      ...
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 space-y-4">
        <h3 className="text-sm font-extrabold text-slate-900 uppercase tracking-wider">UPCOMING 30-DAY SCHEDULE</h3>
        ...
        {dutyDays.length === 0 ? (
          <div className="p-8 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-200 space-y-2">
            ...
            <p className="text-[11px] text-slate-500 max-w-sm mx-auto">
              {currentRole === 'wife'
                ? 'Please enter your 2-day shift baseline above to calculate your 30-day rotation!'
                : 'Waiting for wife to configure shift baseline. You can set your WFH days above!'}
            </p>
          </div>
        ) : (
